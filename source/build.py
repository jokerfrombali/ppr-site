...
def main():
    ...
    for p in ALL:
        page = {'url': p['url'], 'title': p['title'], 'desc': p['desc'], 'h1': p['h1'],
                ...
                'final_h2': p.get('final_h2', 'Рассчитать стоимость разработки'),
                'final_text': p.get('final_text', 'Пришлите задание — посчитаем объём разделов, назовём срок и цену. Расчёт бесплатный, ни к чему не обязывает.')}
        write(page)
    # sitemap
    ...
    open(os.path.join(DIST, '404.html'), 'w', encoding='utf-8').write(_render({
        ...
        'final_h2': 'Рассчитать стоимость разработки',
